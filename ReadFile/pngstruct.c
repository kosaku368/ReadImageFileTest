...
#define PNG_CYCLIC_REDUNDANCY_CHACK_BYTE ( 4)
#define PNG_END_CHUNK_BYTE				 (12)
...
void read_bmp(const char *argv)
{
	...
	if (read_file != NULL)
	{
		if (buffer != NULL) {
			...
			printf(" biHeight  ---------->  %08d\n", ib->biHeight);
			printf(" biPlanes  ---------->  %08d\n", ib->biPlanes);
			printf(" biBitCount  -------->  %08d\n", ib->biBitCount);
			printf(" biCompression   ---->  %08d\n", ib->biCompression);
			printf(" biSizeImage   ------>  %08d\n", ib->biSizeImage);
			printf(" biXPelsPerMeter   -->  %08d\n", ib->biXPelsPerMeter);
			printf(" biYPelsPerMeter   -->  %08d\n", ib->biYPelsPerMeter);
			printf(" biClrUsed ---------->  %08d\n", ib->biClrUsed);
			printf(" biClrImportant   --->  %08d\n", ib->biClrImportant);
